...
const MOCK_USERS = [
  // ── Users ────────────────────────────────────────────────────────────────
  { id: 1, username: 'manoj',       email: 'manoj@nrsc.gov.in',            role: 'User',      area: 'Balanagar', division: 'DPFD',  assetCount: 2 },
  { id: 2, username: 'irfansait',   email: 'irfan@nrsc.gov.in',            role: 'User',      area: 'Shadnagar', division: 'ASAG',  assetCount: 1 },
  { id: 4, username: 'suresh',      email: 'suresh@nrsc.gov.in',           role: 'User',      area: 'Shadnagar', division: 'RSAA',  assetCount: 1 },
  // ── Admin ─────────────────────────────────────────────────────────────────
  { id: 3, username: 'admin',       email: 'admin@nrsc.gov.in',            role: 'Admin',     area: null,        division: 'ASCID', assetCount: 0 },
  // ── Area Admins ───────────────────────────────────────────────────────────
  { id: 5, username: 'areaadmin1',  email: 'areaadmin.balanagar@nrsc.gov.in', role: 'AreaAdmin', area: 'Balanagar', division: 'DPFD',  assetCount: 0 },
  { id: 6, username: 'areaadmin2',  email: 'areaadmin.shadnagar@nrsc.gov.in', role: 'AreaAdmin', area: 'Shadnagar', division: 'RSAA',  assetCount: 0 },
];
...
const MOCK_ASSETS = [
  {
    ...
    UserDivision: 'DPFD', GROUP: 'SPFPG', AssetCustodianECNO: 'EC001',
  },
  {
  ...
  },
  {
    ...
    assigned_to: 3, assignedUserName: 'raghavendra', status: 'Assigned',
    UserDivision: 'ASCID', GROUP: 'RSAA', AssetCustodianECNO: 'EC003',
  },
  {
    ...
    UserDivision: 'ASAG', GROUP: 'ASAG', AssetCustodianECNO: 'EC002',
  },
  {
    ...
    UserDivision: 'RSAA', GROUP: 'RSAA', AssetCustodianECNO: 'EC004',
  },
  {
...
export async function login(email, password) {
  if (USE_MOCK) {
    await delay();
    // Mock: accept any password, just check the email exists
    const user = MOCK_USERS.find(u => u.email.toLowerCase() === email.toLowerCase());
    if (!user) throw new Error('No account found with that email. Check your credentials.');
...
export async function signup(email, password, role, area) {
  if (USE_MOCK) {
    await delay();
  ...
  return apiFetch('/auth/signup', {
    method: 'POST',
    body: JSON.stringify({ email, username: email.split('@')[0], password, role, area }),
  });
}
